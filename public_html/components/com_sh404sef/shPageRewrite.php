 ...
 * @version     $Id: shPageRewrite.php 1702 2010-12-11 11:16:09Z silianacom-svn $
 *
 */
...
if (function_exists('shLoadPluginlanguage')) {
  ...
  function shDoTitleTags( &$buffer) {
    ...
    if ($sefConfig->shMetaManagementActivated) {
      ...
      $isHome = shSortUrl($shPageInfo->shCurrentPageNonSef) == shSortUrl(shCleanUpAnchor($shHomeLink));
      ...
      if ($isHome) { // V 1.2.4.t homepage custom tags
        ...
        $sql = 'SELECT id, metadesc, metakey, metatitle, metalang, metarobots FROM #__sh404SEF_meta WHERE newurl = '.$database->Quote(ltrim($shPageInfo->shCurrentPageNonSef, '/'));
      }
      $shCustomTags = null;
      ...
      if (!empty($shCustomTitleTag)) {
        ...
        $prepend = $app->getCfg('sitename'). ' | '; // cci $isHome ? '' : $sefConfig->prependToPageTitle;
      ...
      if (!is_null($shCustomDescriptionTag)) {
        ...
        .htmlspecialchars( shCleanUpDesc($shCustomDescriptionTag), ENT_COMPAT, 'UTF-8').'" />', $buffer);
      }
      if (!is_null($shCustomKeywordsTag)) {
        $buffer = preg_replace( '/\<\s*meta\s+name\s*=\s*"keywords.*\/\>/isU', '<meta name="keywords" content="'
        .htmlspecialchars( shCleanUpDesc($shCustomKeywordsTag), ENT_COMPAT, 'UTF-8').'" />', $buffer);
      }
      ...
      if (strpos( $buffer, '{sh404sef_pageid}') !== false) {
        ...
        $sql = 'SELECT pageid FROM #__sh404sef_pageids WHERE newurl = '.$database->Quote( ltrim($shPageInfo->shCurrentPageNonSef, '/'));
        $database->setQuery($sql);
